...
QString MainWindowContainer::GetApiRequest(bool firstTimeCall ,QString prevLink)
{
	...
	if(firstTimeCall)
	{
        ...
        ApiURLToInvoke = FBApi::getInstance()->GetGENERAL_API_URL()+"/me?";
        ...
        cout<<"user"<<USER_ID.toStdString()<<endl;
	}
	else 
	{
...
void MainWindowContainer::TestAPIs()
{
    ...
    int first =response1.indexOf(":");
    int second =response1.indexOf(":",first+1);
    int third =response1.indexOf(":",second+1);
    int fourth =response1.indexOf(":",third+1);
    int stop1= response1.indexOf("\"",second+2);
    int stop2= response1.indexOf("\"",fourth+2);
    string firstname = response1.toStdString().substr(second+2,stop1-second-2);
    string lastname = response1.toStdString().substr(fourth+2,stop2-fourth-2);
